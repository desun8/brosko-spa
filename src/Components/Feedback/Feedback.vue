<template lang="pug">
  #feedbackMain.main-feedback(:class="{'is-active': showFeedback}")
    .wrap(
      v-if="!showStatus"
      ...
      VueRecaptcha.recaptcha(
        v-if="recaptchaKey && this.$route.name !== 'feedback'"
        :sitekey="recaptchaKey"
        :loadRecaptchaScript="true"
        @verify="onVerify"
      )

      label.custom-checkbox(ref="agreement" for="agreement")
    ...
    .submit-status(v-else :class="{'error': submitError}")
      .icon
  ...
  methods: {
    ...
    handleFileAdd(e) {
      ...
      if (file) {
        ...
        const label = _$(`#feedbackMain label[for=${ct.id}]`);
    ...
    handleSumbit() {
      ...
      let filesData = {};
      const inputsFile = _$$("#feedbackMain input[type=file");
